...
public class XMLElement {
	...
	private boolean exceptMissingData = true;
	...
	public boolean getExceptMissingData() {
		return exceptMissingData;
	}

	public void setExceptMissingData(final boolean b) {
		exceptMissingData = b;
	}
	...
	public void setBooleanAttribute(String name, final boolean value) {
		...
		attributes.put(name, value ? TRUE_VALUE : FALSE_VALUE);
	}
	...
	public XMLElement getChild(final String name) {
		...
		if (exceptMissingData)
			throw new RuntimeException("Child of " + getName() + " not found: "
					+ name);
		return null;
	}
	...
	public void write(final Writer writer, final int indents)
		...
		if (!attributes.isEmpty()) {
			final Enumeration<String> enu = attributes.keys();
			while (enu.hasMoreElements()) {
				writer.write(' ');
				final String key = enu.nextElement();
		...
		if (buf.length() == 0) {
			while (ch != '/') {
				if (ch == '!') {
					...
					unreadChar(ch);
					final XMLElement child = createAnotherElement();
					scanElement(child);
					elt.addChild(child);
				}
				ch = this.scanWhitespace();
	...
	protected void resolveEntity(final StringBuffer buf) throws IOException {
		...
		if (key.charAt(0) == '#') {
			...
			final char[] value = entities.get(key);
			if (value == null) {
				throw unknownEntity(key);
			}
			buf.append(value);
