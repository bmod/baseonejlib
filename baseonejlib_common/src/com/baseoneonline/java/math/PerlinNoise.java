...
public final class PerlinNoise
{
	...
	static public float noise(float x, float y, float z)
	{
		...
		z -= Math.floor(z);
		final float u = fade(x), // COMPUTE FADE CURVES
		v = fade(y), // FOR EACH OF X,Y,Z.
		w = fade(z);
		final int A = p[X] + Y, AA = p[A] + Z, AB = p[A + 1] + Z, // HASH
																	// COORDINATES
																	// OF
	...
	static float fade(final float t)
	{
		return t * t * t * (t * (t * 6 - 15) + 10);
	}

	static float lerp(final float t, final float a, final float b)
	{
		return a + t * (b - a);
	}

	static float grad(final int hash, final float x, final float y,
			final float z)
	{
		final int h = hash & 15; // CONVERT LO 4 BITS OF HASH CODE
		final float u = h < 8 ? x : y, // INTO 12 GRADIENT DIRECTIONS.
