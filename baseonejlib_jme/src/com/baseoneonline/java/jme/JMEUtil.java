...
public class JMEUtil {
	...
	public static Spatial[] mousePick(Node node) {
		DisplaySystem disp = DisplaySystem.getDisplaySystem();
		Vector2f mouseScreen = new Vector2f(MouseInput.get().getXAbsolute(),
				MouseInput.get().getYAbsolute());
		Vector3f origin = disp.getWorldCoordinates(mouseScreen, 0);
		Vector3f target = disp.getWorldCoordinates(mouseScreen, 1);
		Vector3f direction = target.subtractLocal(origin).normalizeLocal();
		PickResults pickResults = new TrianglePickResults();
		Ray ray = new Ray(origin, direction);
		node.findPick(ray, pickResults);
		int num = pickResults.getNumber();
		
		Spatial[] spatials = new Spatial[num];
		for (int i = 0; i < num; i++) {
			spatials[i] = pickResults.getPickData(i).getTargetMesh();
		}
		return spatials;
	}
