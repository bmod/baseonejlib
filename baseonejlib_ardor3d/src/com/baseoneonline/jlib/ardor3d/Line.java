...
public class Line extends Mesh
{
	...
	/**
	 * Constructor instantiates a new <code>Line</code> object with a given set
	 * of data. Any data can be null except for the vertex list. If vertices are
			...
			final ReadOnlyVector2[] texture, final IndexMode indexMode)
	{
		...
		setupData(BufferUtils.createFloatBuffer(vertex),
				...
				FloatBufferDataUtil.makeNew(texture));
			...
			final double y, final int segments, final boolean insideOut)
	{
		...
		final FloatBuffer normals = BufferUtils.ensureLargeEnough(
				_meshData.getNormalBuffer(), requiredFloats);
		_meshData.setNormalBuffer(normals);
		double angle = 0;
	 ...
	 * with a source of SourceFunction.SourceAlpha and a destination of
