...
public class TrailMesh extends Mesh {
	 ...
	 */
	public void setTrailFront(final ReadOnlyVector3 position,
			final float width, final float tpf) {
	 ...
	 */
	public void setTrailFront(final ReadOnlyVector3 position,
			final ReadOnlyVector3 tangent, final double width, final double tpf) {
	...
	private void updateStep(final ReadOnlyVector3 camPos) {
		...
		for (int i = 0; i < nrTrailSections; i++) {
			...
			if (facingMode == FacingMode.Billboard) {
				...
				trailDirection.normalizeLocal().multiplyLocal(
						trailData.width * 0.5);
			} else if (trailData.tangent != null) {
				trailDirection.set(trailData.tangent).multiplyLocal(
						trailData.width * 0.5);
			} else {
	...
	private void updateInterpolate(final ReadOnlyVector3 camPos) {
		...
		for (int i = 0; i < nrTrailSections; i++) {
			...
			if (i > 0) {
				interpolationVector.lerpLocal(trailVectors.get(i - 1).position,
						throttle);
			}
		}
	...
	private void setData() {
		getMeshData().setVertexBuffer(
				BufferUtils.createVector3Buffer(
						getMeshData().getVertexBuffer(), trailVertices));
		getMeshData().setNormalBuffer(
				BufferUtils.createVector3Buffer(
						getMeshData().getNormalBuffer(), trailVertices));
	...
	private void setTextureData() {
		if (getMeshData().getTextureCoords(0) == null) {
			final FloatBuffer tex = BufferUtils
					.createVector2Buffer(trailVertices);
	...
	private void setIndexData() {
		...
		if (getMeshData().getIndexBuffer() == null) {
			getMeshData().setIndexBuffer(
					BufferUtils.createShortBuffer(trailVertices));
	...
	/**
	 * Get the mesh data to modify it manually. If data is modified,
	 * invalidate() method call is required.
	 * 
	 * @return
	 */
