...
public class Main extends SimpleGame {
	...
	protected void simpleInitGame() {
		...
		for (int i = 0; i < 10; i++) {
			...
			c.pos.x =
				FastMath.nextRandomFloat() * road.getWidth() - road.getWidth()
					/ 2;
	...
	protected void simpleUpdate() {
		for (Car c : cars) {
			...
			c.setLocalRotation(road.getOrientation(c.pos, Vector3f.UNIT_Y));
		}

		Vector3f behindCarPos =
			road.getPoint(followCar.pos.add(new Vector2f(0, -.1f))).add(0, 5, 0);
